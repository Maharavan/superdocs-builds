...
class BibleService:
    ...
    async def add_version(self, series_id: UUID, fact: ExtractedFact, supersedes: BibleFact | None = None) -> BibleFact:
        ...
        record = BibleFact(series_id=series_id, kind=fact.kind.value, entity=fact.entity, attribute=fact.attribute, value=fact.value, version=version, is_current=True, supersedes_id=supersedes.id if supersedes else None, confidence=fact.confidence)
